...
def household_has_eligible_member(household): return (
    Exists(household, eligible_member(_, household))
)


# High-level eligibility
def eligible_member(person, household): return (
    Or(is_eligible_cash_benefit(person, household),
...
def is_eligible_cash_benefit(person, household): return (
    And(under_19(person),
...
def is_child(person, household): return (
    And(age(person) <= 16,
        Exists(household, relationship(person, _) == 'Child'))  # How should this function work?
)


def is_parent(person, household): return (
    Exists(household, Or(relationship(person, _) == "Parent",
...
def num_children(person, household): return (
    Count_if(household, And(age(person) < 18, is_child(person, household))) # How should this function work?
)


# Get count of adults
def num_adults(person, household): return (
    Count_if(household, Or(age(person) > 18, is_parent(person, household)))
)
